...
def labels2contours(labels: np.ndarray, simplify_error: float = 0, return_inner_contours: bool = False) -> list:
    '''
    ...
    contours = [
        positions2contour(p, simplify_error=simplify_error, return_inner_contours=return_inner_contours)
        for p in listPoints
        if len(p) > 0
    ]
    return contours
